      ...
      >
        ...
        <el-table-column align="left" label="按钮组">
          <template #default="scope">
            <el-button type="text" icon="edit" size="small" class="table-button" @click="updateSysDepartment(scope.row)">变更</el-button>
...
export default {
  ...
  methods: {
    ...
    async onDelete() {
      ...
      const res = await deleteSysDepartmentByIds({ ids })
      if (res.code === 0) {
        this.$message({
          type: 'success',
          message: '删除成功'
    ...
    async updateSysDepartment(row) {
      const res = await findSysDepartment({ ID: row.ID })
      this.type = 'update'
      if (res.code === 0) {
        this.formData = res.data.reSysDp
    ...
    async deleteSysDepartment(row) {
      const res = await deleteSysDepartment({ ID: row.ID })
      if (res.code === 0) {
        this.$message({
          type: 'success',
          message: '删除成功'
    ...
    async enterDialog() {
      ...
      switch (this.type) {
        case 'create':
          res = await createSysDepartment(this.formData)
          break
        case 'update':
          res = await updateSysDepartment(this.formData)
          break
        default:
          res = await createSysDepartment(this.formData)
          break
