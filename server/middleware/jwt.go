...
func JWTParse(c *gin.Context, token string) {
	j := utils.NewJWT()
...
func CasdoorParse(c *gin.Context, token string) error {
	claims, err := auth.ParseJwtToken(token)
	if err != nil {

	} else {
		cusClaims := request.CustomClaims{}
...
func JWTAuth() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if err := CasdoorParse(c, token); err != nil {
			JWTParse(c, token)
		}
	}
}
